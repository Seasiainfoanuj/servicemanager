...
var defaults = {
	...
	titleFormat: {
		...
		resourceWeek: "d[ MMM][ yyyy]{ '&#8212;' d MMM yyyy}",
		resourceNextWeeks: "MMM d[ yyyy]{ '&#8212;'[ MMM] d yyyy}",
		resourceDay: 'dddd, d MMM yyyy'
	},
	columnFormat: {
		month: 'ddd',
		week: 'ddd d/MM',
		...
		resourceDay: 'H:mm',
		resourceMonth: 'd ddd',
		resourceWeek: 'ddd d/MM',
		resourceNextWeeks: 'ddd d/MM'
	},
	...
	buttonText: {
		prev: "<span class='fc-button-content'>&lsaquo;</span>",
		next: "<span class='fc-button-content'>&rsaquo;</span>",
		prevYear: "<span class='fc-button-content'>&laquo;</span>",
		nextYear: "<span class='fc-button-content'>&raquo;</span>",
		today: 'Today',
		month: 'Month',
		week: 'Week',
		day: 'Day',
...
var rtlDefaults = {
	...
	buttonText: {
		prev: "<span class='fc-button-content'>&rsaquo;</span>",
		next: "<span class='fc-button-content'>&lsaquo;</span>",
		prevYear: "<span class='fc-button-content'>&raquo;</span>",
		nextYear: "<span class='fc-button-content'>&laquo;</span>"
	},
	buttonIcons: {
...
setDefaults({
	...
	axisFormat: 'h(:mm)tt',
	timeFormat: {
		agenda: 'h:mm tt dd/MM/yy{ - h:mm tt dd/MM/yy}'
	},
	dragOpacity: {
		agenda: .5
	},
...
function ResourceView(element, calendar, viewName) {
	...
	function buildSkeleton(maxRowCnt, showNumbers) {
		...
		for (i=0; i<maxRowCnt; i++) {
			...
			s +=
				"<tr class='fc-resourcerow-" + id + "'><td class='fc-resourceName'><span class='fc-resourceName-content'>" + resourceName + "</span></td>";
			for (j=0; j<colCnt; j++) {
				s +=
...
function DayEventRenderer() {
	...
	function daySegHTML(segs) { // also sets seg.left and seg.outerWidth
		...
		for (i=0; i<segCnt; i++) {
			...
			classes = classes.concat(event.className);
			classes = classes.concat(event.statusClassName);
			if (event.source) {
				classes = classes.concat(event.source.className || []);
			}
			url = event.url;
			popover_title = event.popover_title;
			popover_content = event.popover_content;
			skinCss = getSkinCss(event, opt);
			if (url) {
				if (popover_title && popover_content) {
					html += "<a href='" + htmlEscape(url) + "'" + " rel='popover' data-trigger='hover' data-placement='bottom' title='" + htmlEscape(popover_title) + "' data-content='" + htmlEscape(popover_content) + "'";
				} else {
			...
			if (!event.allDay && seg.isStart || !event.allDay && seg.isEnd) {
				if (right - left > 50) {
					html +=

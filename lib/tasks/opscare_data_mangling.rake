  ...
  namespace :data do
    ...
    task mangle: :environment do
      ...
      %w( users ).each do |table|
        %x[ export PGPASSWORD=#{db['password']}; psql -h #{db['host']} -U #{db['username']} #{db['database']} -c "UPDATE #{table} SET email= email || '.local' WHERE NOT (email LIKE '%.local' OR email LIKE '%@reinteractive.net' OR email LIKE '%@bus4x4.com.au');" ]
      end
    end # task :mangle
  end # namespace :data

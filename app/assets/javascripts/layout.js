...
function hideNav(condition) {

  if (condition == 'hide') {
    $("#left").hide().addClass("forced-hide");
  } else if (condition == 'show') {
    $("#left").show().removeClass("forced-hide");
  } else {
  ...
  if ($("#left").is(":visible")) {
    $("#main").css("margin-left", $("#left").width());
    sessionStorage.setItem('leftNavState', 'visible');
  } else {
    $("#main").css("margin-left", 0);
    sessionStorage.setItem('leftNavState', 'hidden');
  }
...
var ready = function() {
  ...
  if (sessionStorage.leftNavState) {
    if (sessionStorage.leftNavState == 'hidden') {
      hideNav('hide');
    } else {
      hideNav('show');
    };
...
function AjaxInit(){
 if ($("#user_loggedin_role").val() != "masteradmin"){
     $('a.DTTT_button_csv').remove();
     $('a.DTTT_button_print').remove();
  }
}

function HideDelete(){
  if ($("#loggedRole").val() == "superadmin"){
    if (window.location.href.indexOf("/superadmins") >= 0) {

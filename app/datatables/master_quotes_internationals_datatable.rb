class MasterQuotesInternationalsDatatable
...
private
  ...
  def data
    master_quotes.where(:international => 'international').map do |quote|
      [
        name(quote),
        quote.type.name,
        quote.vehicle_make,
  ...
  def view_link(quote)
    link_to('<i class="icon-search"></i>'.html_safe, {controller: "master_quotes_internationals", action: "show", id: quote.id}, {:title => 'View', :class => 'btn action-link'})
  end

  def edit_link(quote)
    if can? :update, MasterQuote
      link_to('<i class="icon-edit"></i>'.html_safe, {controller: "master_quotes_internationals", action: "edit", id: quote.id}, {:title => 'Edit', :class => 'btn action-link'})
    end
  end

  def destroy_link(quote)
    if can? :destroy, MasterQuote
      link_to '<i class="icon-ban-circle"></i>'.html_safe, {controller: "master_quotes_internationals", action: "destroy", id: quote.id}, method: :delete, :class => 'btn action-link', :title => 'Destroy', 'rel' => 'tooltip', data: {confirm: "You are about to permanently delete quote number #{quote.name}. You cannot reverse this action. Are you sure you want to proceed?"}
    end
  end
